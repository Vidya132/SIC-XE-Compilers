...
//tokenization ==> each line is given tokens as label,opcode,operands
...
int pass_1()
{
	...
	while(fin.good())
	{
		...
		instruction instr;
		//tokenization into labels, opcodes, operands
		...
		if(!instr.is_comment())
		{
			...
			{
				STADDR = stoi(instr.operand, NULL, 16);//addr value taken as hex converted to decimal 
				LOCCTR = STADDR;
				//format_number=> decimal value of LOCCTR converted to string 
				//false is because the num is not in decimal(it is in hexadecimal)
				//and it converts to decimal
				//4 => integer width 
				//format_name =>  name padded with space total size of 10
			...
			else
			{
				if(instr.has_label())
				{
					//mapping label in symbol table to it's address
				...
				else if(instr.opcode == "BYTE")
				{
					if(instr.operand[0] == 'C')
						//cout<<"For C "<<" LOCCTR= "<<format_name(format_number(to_string(LOCCTR), 4, false), 10)<<" operand_length()= "<<instr.operand.length()<<endl;
						LOCCTR += (instr.operand.length() - 3);
					if(instr.operand[0] == 'X')
						//cout<<"For X "<<" LOCCTR= "<<format_name(format_number(to_string(LOCCTR), 4, false), 10)<<" operand_length()= "<<instr.operand.length()<<endl;
...
void pass_2(int LENGTH)
{
	...
	while(fin.good())
	{
		...
		if(!instr.is_comment())
		{
			...
			{
				stringstream header;
				header << "H"; //header record starts with H
				header << format_name(instr.label, 6);
				//format_number is padded with 0s
